...
class ExcelController extends Controller
{
    public function import(Request $request, ImportedFile $filedata, User $user, Excelconfig $config)
    {
        try {
            ...
            if ($numSheets > 1) {
                return redirect()->back()->with('error', 'Excel file has more than one sheet. Please upload a file with only one sheet.');
            }
    ...
    {
        $file = $request->file('file');
        $filename = 'main_file' . '.' . $file->getClientOriginalExtension();
    ...
    public function export($filedata, $user, $config)
    {
        ...
        $file_path = base_path("/storage/app/main_file/$getfile_name");
        if (file_exists($file_path)) {
            $file = $file_path;
        }
        $file = base_path("/storage/app/main_file/main_file.xls");
        ...
        $payment_date = $filedata::all()->pluck('payment_date');
        $room = "Room" . " " . rand(1, 19);
        ...
        if ($zip->open(storage_path('app/' . $zip_name), ZipArchive::CREATE) === TRUE) {
            for ($i = 1; $i <= $num_copies; $i++) {
                ...
                $_guestNameRow = $config->where('id', 1)->pluck('guest_name')[0] ?? 'A21';
                $_invoiceNumRow = $config->where('id', 1)->pluck('invoice_number')[0] ?? 'B15';
                $_dateRow = $config->where('id', 1)->pluck('payment_date')[0] ?? 'B17';
                $_roomRow = $config->where('id', 1)->pluck('room')[0] ?? 'B21';
                $_quantity = $config->where('id', 1)->pluck('nights_stayed')[0] ?? 'C21';
                $_unitPrice = $config->where('id', 1)->pluck('unit_price')[0] ?? 'D21';
                $_amountPrice = $config->where('id', 1)->pluck('amount_price')[0] ?? 'E21';

                $_amountTotal = $config->where('id', 1)->pluck('amount_price_row_total')[0] ?? 'D34';
                $_unitTotal = $config->where('id', 1)->pluck('unit_price_total')[0] ?? 'E34';
                ...
                $spreadsheet->getActiveSheet()->setCellValue($_dateRow, $payment_date[$message_index]);
                ...
                $spreadsheet->getActiveSheet()->setCellValue($_unitPrice, ((float)$price[$message_index] / (int)$nights_stayed[$message_index]) ?? null);

                $spreadsheet->getActiveSheet()->setCellValue($_amountPrice, $price[$message_index] ?? null);
                $spreadsheet->getActiveSheet()->setCellValue($_unitTotal, ((float)$price[$message_index] / (int)$nights_stayed[$message_index]) ?? null);
